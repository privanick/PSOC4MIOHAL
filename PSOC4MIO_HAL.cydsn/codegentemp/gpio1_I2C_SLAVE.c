...
volatile uint8 gpio1_slStatus;          /* Slave Status */

/* Receive buffer variables */
volatile uint8 * gpio1_slWrBufPtr;      /* Pointer to Receive buffer  */
volatile uint32  gpio1_slWrBufSize;     /* Slave Receive buffer size  */
volatile uint32  gpio1_slWrBufIndex;    /* Slave Receive buffer Index */

/* Transmit buffer variables */
volatile uint8 * gpio1_slRdBufPtr;      /* Pointer to Transmit buffer  */
volatile uint32  gpio1_slRdBufSize;     /* Slave Transmit buffer size  */
volatile uint32  gpio1_slRdBufIndex;    /* Slave Transmit buffer Index */
volatile uint32  gpio1_slRdBufIndexTmp; /* Slave Transmit buffer Index Tmp */
volatile uint8   gpio1_slOverFlowCount; /* Slave Transmit Overflow counter */

#if (gpio1_I2C_CUSTOM_ADDRESS_HANDLER_CONST)

    uint32 (*gpio1_customAddressHandler)(void) = NULL;

    /*******************************************************************************
    * Function Name: gpio1_SetI2cAddressCustomInterruptHandler
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void gpio1_SetI2cAddressCustomInterruptHandler(uint32 (*func) (void))
    {
        ...
        gpio1_customAddressHandler = func;
    }
#endif /* (gpio1_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */


/*******************************************************************************
* Function Name: gpio1_I2CSlaveStatus
****************************************************************************//**
*
...
*   - gpio1_I2C_SSTAT_RD_CMPLT - Slave read transfer complete. 
...
*   - gpio1_I2C_SSTAT_RD_BUSY - Slave read transfer is in progress. 
*     Set when master addresses slave with a read, cleared when 
*    gpio1_I2C_SSTAT_RD_CMPLT is set.
*   - gpio1_I2C_SSTAT_RD_OVFL - Master attempted to read more bytes
*     than are in buffer. Slave continually returns 0xFF byte in this case.
*   - gpio1_I2C_SSTAT_RD_ERR - Slave captured error on the bus during 
*     a read transfer. The sources of error are: misplaced Start or Stop 
*     condition or lost arbitration while slave drives SDA.
*   - gpio1_I2C_SSTAT_WR_CMPLT - Slave write transfer complete. 
...
*     successfully.
*   - gpio1_I2C_SSTAT_WR_BUSY - Slave write transfer is in progress. 
*     Set when the master addresses the slave with a write, cleared when 
*    gpio1_I2C_SSTAT_WR_CMPLT is set.
*   - gpio1_I2C_SSTAT_WR_OVFL - Master attempted to write past end of 
*     buffer. Further bytes are ignored.
*   - gpio1_I2C_SSTAT_WR_ERR - Slave captured error on the bus during
...
*     gpio1_I2C_SSTAT_WR_ERR is set. It is recommended to discard write 
...
{
    return((uint32) gpio1_slStatus);
}


/*******************************************************************************
* Function Name: gpio1_I2CSlaveClearReadStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear gpio1_I2C_SSTAT_RD_BUSY.
*
* \globalvars
*  gpio1_slStatus  - used to store current status of I2C slave.
*
...
uint32 gpio1_I2CSlaveClearReadStatus(void)
{
    ...
    gpio1_DisableInt();  /* Lock from interruption */

    /* Mask of transfer complete flag and error status */
    status = ((uint32) gpio1_slStatus & gpio1_I2C_SSTAT_RD_MASK);
    gpio1_slStatus &= (uint8) ~gpio1_I2C_SSTAT_RD_CLEAR;

    gpio1_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: gpio1_I2CSlaveClearWriteStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear gpio1_I2C_SSTAT_WR_BUSY.*
*
* \globalvars
*  gpio1_slStatus  - used to store current status of I2C slave.
*
...
uint32 gpio1_I2CSlaveClearWriteStatus(void)
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = ((uint32) gpio1_slStatus & gpio1_I2C_SSTAT_WR_MASK);
    gpio1_slStatus &= (uint8) ~gpio1_I2C_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: gpio1_I2CSlaveSetAddress
****************************************************************************//**
*
...
void gpio1_I2CSlaveSetAddress(uint32 address)
{
    ...
    matchReg = gpio1_RX_MATCH_REG;

    matchReg &= ((uint32) ~gpio1_RX_MATCH_ADDR_MASK); /* Clear address bits */
    matchReg |= ((uint32)  gpio1_GET_I2C_8BIT_ADDRESS(address));

    gpio1_RX_MATCH_REG = matchReg;
}


/*******************************************************************************
* Function Name: gpio1_I2CSlaveSetAddressMask
****************************************************************************//**
*
...
void gpio1_I2CSlaveSetAddressMask(uint32 addressMask)
{
    ...
    matchReg &= ((uint32) ~gpio1_RX_MATCH_MASK_MASK);
    matchReg |= ((uint32) (addressMask << gpio1_RX_MATCH_MASK_POS));
...
*
* \globalvars
*  gpio1_slRdBufPtr   - used to store pointer to slave read buffer.
*  gpio1_slRdBufSize  - used to store salve read buffer size.
*  gpio1_slRdBufIndex - used to store current index within slave
*  read buffer.
*
*******************************************************************************/
void gpio1_I2CSlaveInitReadBuf(uint8 * rdBuf, uint32 bufSize)
{
    ...
    if(NULL != rdBuf)
    {
        ...
        gpio1_slRdBufPtr      = (volatile uint8 *) rdBuf; /* Set buffer pointer  */
        gpio1_slRdBufSize     = bufSize;                  /* Set buffer size     */
        gpio1_slRdBufIndex    = 0u;                       /* Clear buffer index  */
        gpio1_slRdBufIndexTmp = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: gpio1_I2CSlaveInitWriteBuf
****************************************************************************//**
*
*  Sets the buffer pointer and size of the write buffer. This function also 
*  resets the transfer count returned with the 
*  gpio1_I2CSlaveGetWriteBufSize() function.
*
...
*
* \globalvars
*  gpio1_slWrBufPtr   - used to store pointer to slave write buffer.
*  gpio1_slWrBufSize  - used to store salve write buffer size.
...
void gpio1_I2CSlaveInitWriteBuf(uint8 * wrBuf, uint32 bufSize)
{
    ...
    if(NULL != wrBuf)
    {
        ...
        gpio1_slWrBufPtr   = (volatile uint8 *) wrBuf; /* Set buffer pointer  */
        gpio1_slWrBufSize  = bufSize;                  /* Set buffer size     */
        gpio1_slWrBufIndex = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: gpio1_I2CSlaveGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes read by the I2C master since an
*  gpio1_I2CSlaveInitReadBuf() or 
*  gpio1_I2CSlaveClearReadBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if gpio1_I2C_SSTAT_RD_ERR was 
...
{
    return(gpio1_slRdBufIndex);
}


/*******************************************************************************
* Function Name: gpio1_I2CSlaveGetWriteBufSize
****************************************************************************//**
*
*  Returns the number of bytes written by the I2C master since an
*  gpio1_I2CSlaveInitWriteBuf() or 
*  gpio1_I2CSlaveClearWriteBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if gpio1_I2C_SSTAT_WR_ERR was 
*  captured by the slave.
*
* \globalvars
*  gpio1_slWrBufIndex - used to store current index within slave
...
{
    return(gpio1_slWrBufIndex);
}


/*******************************************************************************
* Function Name: gpio1_I2CSlaveClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: gpio1_I2CSlaveClearWriteBuf
****************************************************************************//**
*
